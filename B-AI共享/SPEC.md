...
两实例均设置 `autoDownload=false`、`autoInstallOnAppQuit=false`、`allowPrerelease=false`；设置 channel 后再次明确 `allowDowngrade=false`。（已被 §11.1 修订：`allowPrerelease` **不再显式写 false** —— 交给 electron-updater 按当前版本形态自动决定；壳版本已是 rc 时写死 false 会导致 GitHub 源报 `No published versions on GitHub`。）
...
| 2026-09-25 | **§7 修订：allowPrerelease 改为上游自动决定** | 病征：装好的应用报 `source github failed **No published versions on GitHub**`（**能连上**但找不到版本，与网络失败不同）。根因链（源码级）：① `electron-updater/out/AppUpdater.js:218` 的默认行为是 `this.allowPrerelease = hasPrereleaseComponents(currentVersion)` ⇒ 当前版本是预发布时默认 **true**；② 我们此前**显式写 `allowPrerelease: false`** 覆盖了这个智能默认；③ 于是 GitHubProvider 走 `/releases/latest`，而该端点**只返回非预发布版本**，我们唯一的 Release `v0.1.7-rc.2` 因 tag 含 `-rc.2` 被 GitHub 视为预发布 ⇒ 找不到。⚠ 本质是"后一个决策让前一个决策的前提失效"：§7 定 `allowPrerelease=false` 时假设壳版本是 `0.1.0`（正式版），后来按官方铁律把壳版本改为**绑定 dsh 的 rc 版本** ⇒ 假设失效。⇒ 修法：**删掉显式值**，让 electron-updater 自动决定（rc ⇒ true 能发现更高的 rc；将来壳版本变正式版 ⇒ 自动 false）。**为何"自动"优于"硬写 true"**：硬写 true 会让将来的正式版客户端也接受 alpha/beta，属放宽过头。§7 冻结句已加"已被 §11.1 修订"标注；断言：src/updater.js 不得再出现 `allowPrerelease:`，且注释须写明自动决定的依据（`hasPrereleaseComponents`） | `src/updater.js`、`tests/acceptance.test.mjs` |
| 2026-09-25 | **dshBuildDirty 在 CI 上误报（定位并修）** | 病征：COS 的 `cn-stable.yml` 里 `dshBuildDirty: true`，而 CI 工作区本应干净 ⇒ 元数据谎称"从脏工作区构建"，误导排障。**定位**（逐处排查写文件动作 + 本地复现对照）：CI 里唯一写到**仓库根**的文件是 `release.yml` 的 Release 说明临时文件 `notes.md`（`Out-File -FilePath notes.md`），它未被 gitignore ⇒ `git status --porcelain` 非空 ⇒ dirty 判定为真。**排除**了"构建产物污染"这一猜想：`node_modules/`、`dist/`、`build/` 均已在 `.gitignore`（本地跑完构建后 `git status` 仍干净，证据一致）。⇒ 修法：该临时文件改写 `dist/notes.md`（已被忽略），并加断言"workflow 的所有 `Out-File` 目标必须是 CI 输出变量或已忽略目录"。**未采用**"直接不写该字段"的做法（那会丢失可溯源性）；另加一致性断言：干净工作区下 `dirty` 必须为 false | `.github/workflows/release.yml`、`tests/acceptance.test.mjs` |
| 2026-09-25 | **产物名不含空格 + 本地干跑路径（CI 首次发布失败）** | 病征：CI「同步 COS」步骤 `release: dist/ 缺失产物: DSH-Desktop-Setup-0.1.7-rc.2.exe`（连字符），而磁盘上是 `DSH Desktop-Setup-…`（空格）。因果：`productName "DSH Desktop"` 带空格 → `artifactName` 用 `${productName}` → 磁盘名带空格；**electron-builder 写 latest.yml 时做 URL 安全规范化（空格→连字符）** → 而 release.mjs 按元数据里的 path 找磁盘文件 ⇒ 找不到。**本地从未暴露**：release.mjs 完整流程需 COS 凭据，此前只测过纯函数与"缺凭据退出"。⇒ 修法：①**根本解法** `artifactName` 改为 `DSH-Desktop-Setup-${version}.${ext}`（硬编码连字符，不再用 `${productName}`；`productName` 保留带空格的显示名），工作流的 GitHub 上传 glob 同步为 `dist/DSH-Desktop-Setup-*`；②release.mjs 的产物查找**以磁盘为准兜底**（列举 `dist/` 实际 exe/blockmap，按"空格↔连字符"归一化比对；两边都对不上时报错**同时给出元数据名与磁盘名**）；③断言：artifactName 不含空格、元数据名与磁盘名不一致时仍能匹配且 key 用元数据名；④**新增本地干跑 `node tools/release.mjs --plan-only`**（只读 dist/ 与 latest.yml、构建计划并校验名字一致，**不需凭据、不上传**）—— 这类"只有 CI 才暴露"的问题从此本地可拦 | `electron-builder.yml`、`tools/release.mjs`、`.github/workflows/release.yml`、`tests/acceptance.test.mjs` |
