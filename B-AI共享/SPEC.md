...
| 2026-09-25 | 构建环境依赖：electron-builder 二进制镜像 | `pnpm build` 在无镜像时失败：`connect ETIMEDOUT 20.205.243.166:443`（github.com 不可达），它在下载 winCodeSign/NSIS/7zip。**`.npmrc` 里的 `electron_builder_binaries_mirror` electron-builder 不读**，只认同名环境变量；加上后构建通过。⇒ 已写入 README「安装与开发」并在 `.github/workflows/release.yml` 的构建步骤加同名 env（留空 = 官方源）。换机器 / 新 CI 直接照 README 设 | `README.md`、`.github/workflows/release.yml` |
