...
| 核心方法论 | **Spec-Driven 瀑布式外包**：国际版 GPT-6 只产 Spec → 国内版按 Spec 批量生成代码 → Claude Code（**DeepSeek V4.1 + GLM-5.2 双模型**）跑命令/编译/测试/修错 → 国际版做最终审计 |
...
| 资源 | 可用模型 | 上下文 | 思考强度怎么调 |
|---|---|---|---|
| **Claude Code** | **DeepSeek V4.1** + **GLM-5.2**（双模型可切） | GLM-5.2 达 **1M token** | `/effort` 五档：low · medium · high · **xhigh(显示为 Extra)** · max，外加 Ultracode 模式 |
...
| P1 批量出码 | 国内版 · **GLM-5.2** | **medium（关深度思考）** | 按图纸砌砖，不需要深推理；关掉思考能省近一半时间和积分 |
...
| P2.3 跑通 dev | Claude Code · **GLM-5.2** | **xhigh（Extra）** | 要通读整个项目 + 反复调工具；1M 上下文能一次吃下全部代码，xhigh 正是官方推荐的"长时间代理式任务"档 |
...
| （可选）大规模重构 | Claude Code · GLM-5.2 | **Ultracode** | xhigh + 自动多代理扇出，适合跨几十个文件的重构；**仅当前会话有效** |

### Claude Code 双模型怎么选（你新接的 GLM-5.2 很关键）

- **GLM-5.2 的杀手锏是 1M 上下文 + 长程任务定位** → 凡是"需要通读整个项目"或"跨文件连续改造"的活都给它。
...
| 国际版应急 | 1 次 | ~20 积分 |
| 国内版 · P1 批量出码（GLM-5.2, medium） | ~10 个模块 | ~600 积分 |
| 国内版 · P4 文档 | 1~2 次，low | ~100 积分 |
| 国内版应急返工 | — | ~150 积分 |
| Claude Code · DeepSeek V4.1（medium/high 混用） | ~40~60 轮 | ~¥10~25 |
| Claude Code · GLM-5.2（xhigh 通读 + 攻坚） | ~10~15 轮 | ~¥8~20 |
...
> 剩余的 2300 积分可用来跑二次迭代、做别的项目、或攒到下一次大改造。
