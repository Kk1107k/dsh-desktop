...
# 七审 · 人工验收通过 + 手动下载入口需求（2026-09-25 19:28）

## 一、人工验收 1–4 全部通过（凯哥实测）

| # | 项 | 结果 |
|---|---|---|
| 1 | 推送本地提交 | ✅ |
| 2 | 托盘「检查更新…」（有反馈）+ 弹窗 [关闭]/[重试]（窗口真的关/重进 checking） | ✅ |
| 3 | 托盘绿色徽章 + UI 完整渲染 | ✅ |
| 4 | NSIS 卸载流程（`Uninstall DSH Desktop.exe`） | ✅ |

⇒ **`dist/DSH Desktop-Setup-0.1.0.exe` 在真机上完成了"安装 → 使用 → 卸载"的完整闭环**；
§12 的 A09（安装/卸载/启动）由"需真机验证"转为 **已验通过**。

## 二、新需求：error 态须提供手动下载入口（SPEC §7:251）

用户实测发现：更新失败（"无法连接更新服务"）后，页面只有 [关闭] [重试]，**没有手动下载入口**。

SPEC §7:251 原文要求：双源网络失败显示"无法连接更新服务"，提供原生"手动下载 / 关闭"按钮；
**下载地址固定为 `https://<占位官网域名>/download`，由主进程验证后打开**。

**审计方风险提示（同一陷阱第 3 次出现）**：SPEC 中的地址含 `<占位官网域名>`，**不是合法 URL**。
若照抄进代码，点击时将重演"构造时抛 Invalid URL / openExternal 静默失败"。
⇒ 必须：① 默认值给**真实可用地址**（建议 GitHub Releases 页）
② 做成**可配置**（官网/COS 就绪后切换）③ **打开前验证 http(s) 合法性**（只允许 https）。

**契约影响**：页面须新增"打开下载页"能力 ⇒ 必然新增 IPC channel + preload 方法
⇒ **必须同步更新 `INTERFACE.md`** 与 A07 的契约断言（`updateAPI` 方法集变更）。

**状态**：由"缓至发布准备期"改为**本轮实施**（用户已明确要求）。
...
|---|---|
| 代码层 | **P0 = 0、P1 = 0** |
| 验收面（SPEC §12） | A01~A09 通过；A10 发布时通过 |
| 回归 | 70 条全绿；tsc 0 错 |
| 发布 | `v0.1.7-rc.2` Release + COS 同步成功 |
| 装包实测 | 从 Release 下载 → 装 → 启动 → 托盘，全通 |
| 卸载实测 | NSIS 卸载器，目录清空 |

**审计方自身的误判共记录 7 次**（二审 #11 严重度误判、dev 验证前提错误、
抖动时机多余担心、构建一致性破例、dirty 真因猜错、grep 措辞问题、无视觉能力）。
均已修正，并固化为防御规则（见项目总结报告 §5.2 的 R1/R2）。

**方法论已固化为技能**：`~/.workbuddy/skills/independent-delivery-audit/SKILL.md`
—— 三审制、独立核验六手法、静默失效三形状、审计环境声明、审计独立性护栏。

后续维护与已知限制见：`A-我读的/项目总结报告.md`
